...
contract PaymentsTest is AddressRegistry, TestUtils {
    ...
    function test_latePayments() external {
        ...
        vm.warp(loanV3.nextPaymentDueDate() + 1 days);  // 1 day late
        ...
        vm.warp(loanV3.nextPaymentDueDate() + 2 days);
        ...
        borrower.erc20_approve(USDC, address(loanV3), totalPaid);
        borrower.loan_makePayment(address(loanV3), totalPaid);

        assertEq(loanV3.drawableFunds(),      0);
        assertEq(loanV3.claimableFunds(),     1_000_000 * USD + interestPortion);
        assertEq(loanV3.nextPaymentDueDate(), 0);
        assertEq(loanV3.principal(),          0);
        assertEq(loanV3.paymentsRemaining(),  0);

        assertEq(usdc.balanceOf(address(loanV3)), 1_000_000 * USD + interestPortion);
    ...
    function test_closeLoan() external {
        ...
        ( uint256 principal, uint256 earlyInterest,uint256 delegateFee, uint256 treasuryFee ) = loanV3.getEarlyPaymentBreakdown();
        ...
        assertEq(loanV3.drawableFunds(),      0);
        assertEq(loanV3.claimableFunds(),     principal + earlyInterest);
        assertEq(loanV3.nextPaymentDueDate(), 0);
        assertEq(loanV3.principal(),          0);
        assertEq(loanV3.paymentsRemaining(),  0);
